...
inline TsdfMap::Config getTsdfMapConfigFromRosParam(
  ...
  nh_private.param(ns+"tsdf_voxel_size", voxel_size, voxel_size);
  nh_private.param(ns+"tsdf_voxels_per_side", voxels_per_side, voxels_per_side);
...
inline ICP::Config getICPConfigFromRosParam(const ros::NodeHandle& nh_private, const std::string &ns) {
  ICP::Config icp_config;

  nh_private.param(ns+"icp_min_match_ratio", icp_config.min_match_ratio,
                   icp_config.min_match_ratio);
  nh_private.param(ns+"icp_subsample_keep_ratio", icp_config.subsample_keep_ratio,
                   icp_config.subsample_keep_ratio);
  nh_private.param(ns+"icp_mini_batch_size", icp_config.mini_batch_size,
                   icp_config.mini_batch_size);
  nh_private.param(ns+"icp_refine_roll_pitch", icp_config.refine_roll_pitch,
                   icp_config.refine_roll_pitch);
  nh_private.param(ns+"icp_inital_translation_weighting",
                   icp_config.inital_translation_weighting,
                   icp_config.inital_translation_weighting);
  nh_private.param(ns+"icp_inital_rotation_weighting",
...
inline TsdfIntegratorBase::Config getTsdfIntegratorConfigFromRosParam(
  ...
  const TsdfMap::Config tsdf_config = getTsdfMapConfigFromRosParam(nh_private, ns);
  ...
  nh_private.param(ns+"voxel_carving_enabled",
                   integrator_config.voxel_carving_enabled,
                   integrator_config.voxel_carving_enabled);
  nh_private.param(ns+"truncation_distance", truncation_distance,
                   truncation_distance);
  nh_private.param(ns+"max_ray_length_m", integrator_config.max_ray_length_m,
                   integrator_config.max_ray_length_m);
  nh_private.param(ns+"min_ray_length_m", integrator_config.min_ray_length_m,
                   integrator_config.min_ray_length_m);
  nh_private.param(ns+"max_weight", max_weight, max_weight);
  nh_private.param(ns+"use_const_weight", integrator_config.use_const_weight,
                   integrator_config.use_const_weight);
  nh_private.param(ns+"use_weight_dropoff", integrator_config.use_weight_dropoff,
                   integrator_config.use_weight_dropoff);
  nh_private.param(ns+"allow_clear", integrator_config.allow_clear,
                   integrator_config.allow_clear);
  nh_private.param(ns+"start_voxel_subsampling_factor",
                   integrator_config.start_voxel_subsampling_factor,
                   integrator_config.start_voxel_subsampling_factor);
  nh_private.param(ns+"max_consecutive_ray_collisions",
                   integrator_config.max_consecutive_ray_collisions,
                   integrator_config.max_consecutive_ray_collisions);
  nh_private.param(ns+"clear_checks_every_n_frames",
                   integrator_config.clear_checks_every_n_frames,
                   integrator_config.clear_checks_every_n_frames);
  nh_private.param(ns+"max_integration_time_s",
                   integrator_config.max_integration_time_s,
                   integrator_config.max_integration_time_s);
  nh_private.param(ns+"anti_grazing", integrator_config.enable_anti_grazing,
                   integrator_config.enable_anti_grazing);
  nh_private.param(ns+"use_sparsity_compensation_factor",
                   integrator_config.use_sparsity_compensation_factor,
                   integrator_config.use_sparsity_compensation_factor);
  nh_private.param(ns+"sparsity_compensation_factor",
                   integrator_config.sparsity_compensation_factor,
                   integrator_config.sparsity_compensation_factor);
  nh_private.param(ns+"integration_order_mode",
...
inline EsdfIntegrator::Config getEsdfIntegratorConfigFromRosParam(
  ...
  nh_private.param(ns+"esdf_euclidean_distance",
                   esdf_integrator_config.full_euclidean_distance,
                   esdf_integrator_config.full_euclidean_distance);
  nh_private.param(ns+"esdf_max_distance_m", esdf_integrator_config.max_distance_m,
                   esdf_integrator_config.max_distance_m);
  nh_private.param(ns+"esdf_min_distance_m", esdf_integrator_config.min_distance_m,
                   esdf_integrator_config.min_distance_m);
  nh_private.param(ns+"esdf_default_distance_m",
                   esdf_integrator_config.default_distance_m,
                   esdf_integrator_config.default_distance_m);
  nh_private.param(ns+"esdf_min_diff_m", esdf_integrator_config.min_diff_m,
                   esdf_integrator_config.min_diff_m);
  nh_private.param(ns+"clear_sphere_radius",
                   esdf_integrator_config.clear_sphere_radius,
                   esdf_integrator_config.clear_sphere_radius);
  nh_private.param(ns+"occupied_sphere_radius",
                   esdf_integrator_config.occupied_sphere_radius,
                   esdf_integrator_config.occupied_sphere_radius);
  nh_private.param(ns+"esdf_add_occupied_crust",
...
inline MeshIntegratorConfig getMeshIntegratorConfigFromRosParam(
  ...
  nh_private.param(ns+"mesh_min_weight", mesh_integrator_config.min_weight,
                   mesh_integrator_config.min_weight);
  nh_private.param(ns+"mesh_use_color", mesh_integrator_config.use_color,
