...
TsdfServer::TsdfServer(const ros::NodeHandle& nh,
    ...
    : TsdfServer(nh, nh_private, getTsdfMapConfigFromRosParam(nh_private, ns),
                 getTsdfIntegratorConfigFromRosParam(nh_private, ns),
                 getMeshIntegratorConfigFromRosParam(nh_private, ns), ns) {}
...
TsdfServer::TsdfServer(const ros::NodeHandle& nh,
  ...
  surface_pointcloud_pub_ =
      nh_private_.advertise<pcl::PointCloud<pcl::PointXYZRGB> >(
          ns_+"surface_pointcloud", 1, true);
  tsdf_pointcloud_pub_ =
      nh_private_.advertise<pcl::PointCloud<pcl::PointXYZI> >(ns_+"tsdf_pointcloud",
                                                              1, true);
  occupancy_marker_pub_ =
      nh_private_.advertise<visualization_msgs::MarkerArray>(ns_+"occupied_nodes",
                                                             1, true);
  tsdf_slice_pub_ = nh_private_.advertise<pcl::PointCloud<pcl::PointXYZI> >(
      ns_+"tsdf_slice", 1, true);

  nh_private_.param(ns_+"pointcloud_queue_size", pointcloud_queue_size_,
                    pointcloud_queue_size_);
  pointcloud_sub_ = nh_.subscribe(ns_+"pointcloud", pointcloud_queue_size_,
                                  &TsdfServer::insertPointcloud, this);

  mesh_pub_ = nh_private_.advertise<voxblox_msgs::Mesh>(ns_+"mesh", 1, true);

  // Publishing/subscribing to a layer from another node (when using this as
  // a library, for example within a planner).
  tsdf_map_pub_ =
      nh_private_.advertise<voxblox_msgs::Layer>(ns_+"tsdf_map_out", 1, false);
  tsdf_map_sub_ = nh_private_.subscribe(ns_+"tsdf_map_in", 1,
                                        &TsdfServer::tsdfMapCallback, this);
  nh_private_.param(ns_+"publish_tsdf_map", publish_tsdf_map_, publish_tsdf_map_);
  ...
  if (use_freespace_pointcloud_) {
    ...
    freespace_pointcloud_sub_ =
        nh_.subscribe(ns_+"freespace_pointcloud", pointcloud_queue_size_,
                      &TsdfServer::insertFreespacePointcloud, this);
  }

  if (enable_icp_) {
    icp_transform_pub_ = nh_private_.advertise<geometry_msgs::TransformStamped>(
        ns_+"icp_transform", 1, true);
    nh_private_.param(ns_+"icp_corrected_frame", icp_corrected_frame_,
                      icp_corrected_frame_);
    nh_private_.param(ns_+"pose_corrected_frame", pose_corrected_frame_,
  ...
  nh_private_.param(ns_+"method", method, method);
  ...
  icp_.reset(new ICP(getICPConfigFromRosParam(nh_private, ns_)));

  // Advertise services.
  generate_mesh_srv_ = nh_private_.advertiseService(
      ns_+"generate_mesh", &TsdfServer::generateMeshCallback, this);
  clear_map_srv_ = nh_private_.advertiseService(
      ns_+"clear_map", &TsdfServer::clearMapCallback, this);
  save_map_srv_ = nh_private_.advertiseService(
      ns_+"save_map", &TsdfServer::saveMapCallback, this);
  load_map_srv_ = nh_private_.advertiseService(
      ns_+"load_map", &TsdfServer::loadMapCallback, this);
  publish_pointclouds_srv_ = nh_private_.advertiseService(
      ns_+"publish_pointclouds", &TsdfServer::publishPointcloudsCallback, this);
  publish_tsdf_map_srv_ = nh_private_.advertiseService(
      ns_+"publish_map", &TsdfServer::publishTsdfMapCallback, this);

  // If set, use a timer to progressively integrate the mesh.
  double update_mesh_every_n_sec = 1.0;
  nh_private_.param(ns_+"update_mesh_every_n_sec", update_mesh_every_n_sec,
  ...
  nh_private_.param(ns_+"publish_map_every_n_sec", publish_map_every_n_sec,
...
void TsdfServer::getServerConfigFromRosParam(
  ...
  nh_private.param(ns_+"min_time_between_msgs_sec", min_time_between_msgs_sec,
                   min_time_between_msgs_sec);
  min_time_between_msgs_.fromSec(min_time_between_msgs_sec);

  nh_private.param(ns_+"max_block_distance_from_body",
                   max_block_distance_from_body_,
                   max_block_distance_from_body_);
  nh_private.param(ns_+"slice_level", slice_level_, slice_level_);
  nh_private.param(ns_+"world_frame", world_frame_, world_frame_);
  nh_private.param(ns_+"publish_pointclouds_on_update",
                   publish_pointclouds_on_update_,
                   publish_pointclouds_on_update_);
  nh_private.param(ns_+"publish_slices", publish_slices_, publish_slices_);
  nh_private.param(ns_+"publish_pointclouds", publish_pointclouds_,
                   publish_pointclouds_);

  nh_private.param(ns_+"use_freespace_pointcloud", use_freespace_pointcloud_,
                   use_freespace_pointcloud_);
  nh_private.param(ns_+"pointcloud_queue_size", pointcloud_queue_size_,
                   pointcloud_queue_size_);
  nh_private.param(ns_+"enable_icp", enable_icp_, enable_icp_);
  nh_private.param(ns_+"accumulate_icp_corrections", accumulate_icp_corrections_,
                   accumulate_icp_corrections_);

  nh_private.param(ns_+"verbose", verbose_, verbose_);

  // Mesh settings.
  nh_private.param(ns_+"mesh_filename", mesh_filename_, mesh_filename_);
  std::string color_mode("");
  nh_private.param(ns_+"color_mode", color_mode, color_mode);
  ...
  nh_private.param(ns_+"intensity_colormap", intensity_colormap,
                   intensity_colormap);
  nh_private.param(ns_+"intensity_max_value", intensity_max_value,
                   intensity_max_value);
...
void TsdfServer::processPointCloudMessageAndInsert(
  ...
  Pointcloud points_C;
  Colors colors;
  timing::Timer ptcloud_timer(ns_+"ptcloud_preprocess");
  ...
  if (enable_icp_) {
    timing::Timer icp_timer(ns_+"icp");
...
void TsdfServer::publishMap(bool reset_remote_map) {
  ...
  if (subscribers > 0) {
    ...
    timing::Timer publish_map_timer(ns_+"map/publish_tsdf");
...
void TsdfServer::updateMesh() {
  ...
  timing::Timer generate_mesh_timer(ns_+"mesh/update");
  constexpr bool only_mesh_updated_blocks = true;
  constexpr bool clear_updated_flag = true;
  mesh_integrator_->generateMesh(only_mesh_updated_blocks, clear_updated_flag);
  generate_mesh_timer.Stop();

  timing::Timer publish_mesh_timer(ns_+"mesh/publish");
...
bool TsdfServer::generateMesh() {
  timing::Timer generate_mesh_timer(ns_+"mesh/generate");
  ...
  if (!mesh_filename_.empty()) {
    timing::Timer output_mesh_timer(ns_+"mesh/output");
...
void TsdfServer::tsdfMapCallback(const voxblox_msgs::Layer& layer_msg) {
  timing::Timer receive_map_timer(ns_+"map/receive_tsdf");
